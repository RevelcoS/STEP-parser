...
cmesh4::SimpleMesh cmesh4::LoadMeshFromSTL(const char *filepath, bool &success) {
   stl_reader::StlMesh <float, unsigned int> stlmesh;
   try { stlmesh = stl_reader::StlMesh(filepath); }
...
void cmesh4::SaveMeshToVSGF(const char* a_fileName, const SimpleMesh& a_mesh)
{
...
void cmesh4::SimpleMesh::ApplyMatrix(const LiteMath::float4x4& m)
{
